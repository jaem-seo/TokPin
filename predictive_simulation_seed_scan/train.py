...
from ic_bc_data import ic_bc_data, ic_func
from coefficients import *

n_output = 2 # [10^19/m^3], [keV], [keV]
...
class TransportPinn(object):
    ...
    def save_weights(self, path):
        self.nn.save_weights(path + f'nn{self.seed}.h5')


    def load_weights(self, path):
        self.nn.load_weights(path + f'nn{self.seed}.h5')


    def compute_loss(self, f, u_bnd_hat, u_bnd_sol, u_init_hat, u_init_sol):
        ...
        self.individual_loss = [loss_col.numpy(), loss_bnd.numpy(), loss_init.numpy()]
        
        w1, w2, w3 = self.weights
        loss = w1 * loss_col + w2 * loss_bnd + w3 * loss_init
        return loss
    ...
    def compute_grad(self):
        with tf.GradientTape() as tape:
            ...
            loss = self.compute_loss(f, u_bnd_hat, self.u_bnd_sol, u_init_hat, self.u_init_sol)
    ...
    def callback(self, arg=None):
        ...
        if self.iter_count % self.resample_period == 0:
            self.xt_col, self.xt_bnd, self.u_bnd_sol, self.xt_init, self.u_init_sol = ic_bc_data()
            
        self.iter_count += 1


    def train_with_adam(self, adam_num):
    ...
    def train(self, adam_num, bfgs_num, lbfgs_num, weights=[1, 1, 1], repeat=3, plot=True):
        ...
        np.savetxt(f'./save_weights/loss{self.seed}.txt', self.train_loss_history)
...
if __name__=="__main__":
    ...
    for seed in range(8):
        ...
        agent.train(adam_num, bfgs_num, lbfgs_num, weights=[1000, 1, 1], repeat=1)
        ...
        for i in range(len(ys)):
            ...
            for j in range(n_output):
                ...
                #axs[i].plot(x, u[j, it, :], color=cs[j], ls='--')
            if i == 0:
        ...
        for j in range(n_output):    
            axs[-1].plot(times, [ys[i][0, j] for i in range(len(times))], color=cs[j])
